---
layout: post
title: [转]必须了解的mysql三大日志-binlog、redo log和undo log
categories: [mysql]
description: 必须了解的mysql三大日志-binlog、redo log和undo log
keywords: MySQL,日志
---

前言：日志是`mysql`数据库的重要组成部分，记录着数据库运行期间各种状态信息。`mysql`日志主要包括错误日志、查询日志、慢查询日志、事务日志、二进制日志几大类。作为开发，我们重点需要关注的是二进制日志(`binlog`)和事务日志(包括`redo log`和`undo log`)，本文接下来会详细介绍这三种日志。

原文地址：https://juejin.cn/post/6860252224930070536
...
在计算机操作系统中，用户空间(`user space`)下的缓冲区数据一般情况下是无法直接写入磁盘的，中间必须经过操作系统内核空间(`kernel space`)缓冲区(`OS Buffer`)。因此，`redo log buffer`写入`redo log file`实际上是先写入`OS Buffer`，然后再通过系统调用`fsync()`将其刷到`redo log file`中，过程如下： ![img](2021-2-9-mysql-三大日志.assets/dbc4395ebadf4dbd87e1c64a6bdb68e0_tplv-k3u1fbpfcp-zoom-1.image)
...
![img](2021-2-9-mysql-三大日志.assets/dcfbc08983004721bfa445ea3cc9777d_tplv-k3u1fbpfcp-zoom-1.image)

### redo log记录形式

前面说过，`redo log`实际上记录数据页的变更，而这种变更记录是没必要全部保存，因此`redo log`实现上采用了大小固定，循环写入的方式，当写到结尾时，会回到开头循环写日志。如下图： ![img](2021-2-9-mysql-三大日志.assets/5bd580611e99442a8037c9b5f0b519bc_tplv-k3u1fbpfcp-zoom-1.image)
...
1. [juejin.im/post/684490…](https://juejin.im/post/6844903794073960455)
2. [www.cnblogs.com/f-ck-need-u…](https://www.cnblogs.com/f-ck-need-u/archive/2018/05/08/9010872.html)
3. [www.cnblogs.com/ivy-zheng/p…](https://www.cnblogs.com/ivy-zheng/p/11094528.html)
4. [yq.aliyun.com/articles/59…](https://yq.aliyun.com/articles/592937)
5. [www.jianshu.com/p/5af73b203…](https://www.jianshu.com/p/5af73b203f2a)
6. [www.jianshu.com/p/20e10ed72…](https://www.jianshu.com/p/20e10ed721d0)
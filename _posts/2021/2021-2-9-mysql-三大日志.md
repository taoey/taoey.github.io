---
layout: post
title: mysql-三大日志-binlog、redo log和undo log
categories: [mysql]
description: mysql-三大日志-binlog、redo log和undo log
keywords: MySQL,日志
---
...
![img](http://taoey.github.io/assets/images/artcles/2021-2-9-mysql-三大日志.assets/dcfbc08983004721bfa445ea3cc9777d_tplv-k3u1fbpfcp-zoom-1.image)

### 3、redo log记录形式

前面说过，`redo log`实际上记录数据页的变更，而这种变更记录是没必要全部保存，因此`redo log`实现上采用了大小固定，循环写入的方式，当写到结尾时，会回到开头循环写日志。如下图： ![img](http://taoey.github.io/assets/images/artcles/2021-2-9-mysql-三大日志.assets/5bd580611e99442a8037c9b5f0b519bc_tplv-k3u1fbpfcp-zoom-1.image)
